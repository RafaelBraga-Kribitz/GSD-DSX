# Requirements Archive: v2.4 Visual Excellence
...
- [x] REQ-P23-01 `styles/*.mplstyle` set: dsx-538 (forked from matplotlib,
  Matplotlib License), dsx-urban (2 of 6 palette hexes genuinely Urban's own,
  3 correctly re-attributed to ColorBrewer PRGn/PuOr, 1 Urban gray-ramp stock
  value; Urban's own stated license is GPL-3.0-disputed, not Apache-2.0 —
  moot regardless since bare hex values are not independently copyrightable;
  vendored OFL Lato — house default), dsx-econ and dsx-bbc reimplemented from
  published doctrine only (no GPL port, no unlicensed PDF embedding);
  per-file license/attribution headers; license audit as an explicit
  plan-review item — the audit itself is what surfaced and corrected the
  Urban/ColorBrewer mislabeling (HQ-33, 2026-09-03).
