...
- [x] S7-5 `/gsd-complete-milestone` — **NOT headless-safe** (interactive prompts +
...
- [x] S7-6 Ship: merge to `main` **by explicit branch name**
...
2026-09-10T22:40Z | S7-5 → **DONE (`/gsd-complete-milestone`, interactive; box CHECKED)** | Driven by the operator's session with the standing defects bypassed by hand. Pre-close `audit-open`: 2 dormant seeds (SEED-001, SEED-003 — dormant BY DESIGN, D-13) + 3 Phase-25 CONTEXT bullets the parser reads as open questions (closed veto windows) → acknowledged, recorded in STATE.md `## Deferred Items`, not gaps. `gsd-tools query milestone.complete v2.6` archived ROADMAP/REQUIREMENTS/audit/6 phase dirs to `.planning/milestones/v2.6-*` (16 plans); its accomplishment extraction produced fragments for the FOURTH consecutive close → MILESTONES.md entry hand-written from the phase records; STATE.md hand-corrected. ROADMAP regrouped (v2.6 ✅ details block, 6 progress rows, totals, Active section removed — archived), PROJECT.md full evolution review (v2.6 Shipped section, 6 Validated rows, Active→none, Context, 6 Key Decisions v2.6-01..06, footer), RETROSPECTIVE v2.6 section + trends row. Ledger/queue/ledger-archive `git mv` → `milestones/v2.6-*` (LOOP-BRIEF stays, rewritten at next open). Safety commit by explicit path with plain git (never `gsd-tools query commit`), THEN `git rm .planning/REQUIREMENTS.md`. | milestones/v2.6-*; MILESTONES.md; PROJECT.md; ROADMAP.md; RETROSPECTIVE.md; STATE.md
2026-09-10T22:40Z | S7-6 → **DONE (ship: merge to `main` by EXPLICIT branch name; tag `v2.6.0` on the merge commit; box CHECKED)** | `main` (`d2f0140`) is an ancestor of the branch (113 commits ahead, 0 behind); merge rehearsed on a throwaway branch first, then `git merge --no-ff gsd/v2.6.0-exploration-depth` into `main` (never the framework's alphabetical auto-detect — six stale `gsd/*` branches), full suite + catalogue + `install.mjs --check` re-run ON `main` after the merge, annotated tag `v2.6.0` placed on the merge commit (v2.4.0 precedent), `main` and the tag pushed on the operator's explicit go. The loop stays PAUSED (`.paused`) until v2.7 opens; `$Branch` in `scripts/run-ceremony-firing.ps1` is repointed at that open. This ledger is archived as `milestones/v2.6-LOOP-LEDGER.md` in the same close. | merge commit on `main`; tag `v2.6.0`; origin/main
2026-09-10T23:12Z | S7-6 addendum — first merge into `main` DISCARDED after the post-merge suite failed; five release defects fixed on the branch (`e52d7da`, `0f61eb5`), proven green in a fresh clone, merge redone | On `main` (CRLF checkout) 5 tests failed: the Phase-28/29 fixtures' `evidence`/`narrative.path` pointed at spike NARRATIVE.md files under `.planning/phases/` (archived at S7-5) → DSX-CLM-031 + DSX-NAR-010, four golden tests drifted; the profiler goldens hashed CSV bytes git rewrote as CRLF. A fresh clone then showed two more: the profiler golden pinned this machine's absolute `source_path`; the example-profile byte pins were LF-recorded while the test hashed raw bytes. And a deep clone location overran Windows' 260-char path limit on the v2.6 archive (max 135 chars below root). Fixes: narratives copied as fixture siblings into `examples/known-bad/` and specs repointed; `tests/fixtures/profiler/*.csv -text`; golden `source_path` relative + compared by basename; pins hash LF-normalised bytes (test_frame_val.py precedent); README documents `core.longpaths`. The un-pushed local merge was reset; rehearsal → merge → verify-on-main → tag repeated on the fixed tip. Recorded as PROJECT.md v2.6-07 and in the README Development section. | `e52d7da`, `0f61eb5`; MILESTONES.md; RETROSPECTIVE.md; PROJECT.md; README.md
